<?php if (isset($data)) : ?>
    ...
    $_has_atleast_one_url = $_view_url || $_edit_url || $_delete_url;
    $_empty_message = se($data, "empty_message", "No records to show", false);
    $_header_override = isset($data["header_override"]) ? $data["header_override"] : []; // Note: this is a CSV string or an array
    // Assumes CSV list; explodes to array
    if (is_string($_header_override)) {
        $_header_override = explode(",", $_header_override);
    }
    $_ignored_columns = isset($data["ignored_columns"]) ? $data["ignored_columns"] : []; // Note: this is a CSV string or an array
    // Assumes CSV list; explodes to array
